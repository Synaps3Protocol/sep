...
- The `termsURI` MUST point to a verifiable document. Given the content-addressable nature of decentralized storage systems like IPFS or Arweave, integrity is ensured by the URI itself and termsHash is OPTIONAL. Otherwise, such as in mutable sources (e.g., HTTPS), `termsHash` SHOULD be included to guarantee auditability and MUST match the canonical content of the referenced file.
- Legal interpretation MUST be handled off-chain. For interpretation constraints, see also Section 8.
- Smart contracts SHOULD NOT attempt to interpret or enforce legal terms, which are inherently off-chain and context-dependent.
