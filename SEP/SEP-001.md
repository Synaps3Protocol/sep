...
* JSON Compact Serialization: This is the default serialization format for JWTs. It combines the header, payload, and signature (if present) into a single string, separated by periods. This format is compact and easy to transmit over the wire.
...
As part of the serialization and for interoperability and standardization purposes, the use of raw blocks is established for the storage of the compact serialization of the JWTs and the encoding in dag-jose for its distribution through IPLD
