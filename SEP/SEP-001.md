...
The proposed standard uses JSON Web Tokens (JWT) as the container structure for the metadata. It extends JWTs with additional claims to represent the structural, descriptive, and technical metadata of the resources.
...
To address these challenges, it is imperative to develop a secure standard for metadata structure that ensures interoperability and facilitates the management of digital rights.

## Rationale

1. Why JWT?

Interoperability is one of the main reasons behind use JWT. JWTs are an industry-standard format, which means they are widely recognized and supported by many programming languages, frameworks, and tools.

2. Why does it need to be signed?

By allowing verification of data origin through a digital signature, it becomes possible to provide "proof of ownership". We believe that JSON Web Tokens (JWTs) can be used to encapsulate our current standard, and eventually evolve into a more secure standard, such as through the use of JSON Web Signature ([RFC7515](https://www.rfc-editor.org/rfc/rfc7515)).

Imagine a token that, instead of being validated by a centralized machine, can be verified by different nodes in the network. In this case, the difference in the signature is that the "secret" is not private for verification. Rather, the "secret" is converted to a public key, which serves as an identity that allows us to verify the authenticity of the token by any entity that wants to consume the resource.
