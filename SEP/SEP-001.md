...
> Interoperability is one of the main reasons for using JWTs. JWT is a widely recognized and supported standard in many programming languages, frameworks, and tools. We believe that JSON Web Tokens (JWT) can serve as an encapsulation of our current standard and evolve into a more secure standard through the implementation of JSON Web Signature ([RFC7515](https://www.rfc-editor.org/rfc/rfc7515)) and JSON Web Encrypted ([RFC7516](https://www.rfc-editor.org/rfc/rfc7516)).
...
In the JSON compact serialization format, the "claims" field MUST use a CID value in order to maintain a compact and lightweight design of the JWT signature. The following example demonstrates this:
...
Furthermore, this standard proposes the use of [dag-jose](http://ipld.io.ipns/specs/codecs/dag-jose/spec/) as a complementary standard for the serialization of JSON Web Tokens (JWTs) used to transmit multimedia metadata over decentralized systems. DAG-JOSE provides a more robust and secure serialization format that supports JSON General Syntax serialization and binary encoding formats.
