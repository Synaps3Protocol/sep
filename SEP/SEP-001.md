...
When using the [JSON Compact Serialization](https://www.rfc-editor.org/rfc/rfc7515#section-7.1) format, the `claims` SHOULD use a [CID](https://docs.ipfs.tech/concepts/content-addressing/) value pointing to the corresponding JSON raw block. This approach preserves the compact and lightweight design of the JWT signature while also giving it a deterministic nature. This enables it to be stored in raw blocks plus fast bitswap. eg:
