...
> **Comments:** Consider a token that, instead of being validated by a single machine, can be verified by multiple nodes on the network. The signature of this token differs from others in that the "secret" is not kept private for verification. Instead, the "secret" is converted into a public key that serves as an identity, allowing any entity that wishes to consume the resource to verify the authenticity and ownership of the token.
