...
> Interoperability is one of the main reasons for using JWT standard. JWT is a widely recognized and supported standard in many programming languages, frameworks, and tools. We believe that JSON Web Tokens (JWT) can serve as an encapsulation of our current standard and evolve into a secure standard through the implementation of JSON Web Signature ([RFC7515](https://www.rfc-editor.org/rfc/rfc7515)) and JSON Web Encrypted ([RFC7516](https://www.rfc-editor.org/rfc/rfc7516)).
...
>
> Imagine a scenario in which the signature of a JWT token can be verified by multiple nodes in a network, rather than relying on a centralized machine for validation. This would create a decentralized identity system that allows any entity within the network to verify the authenticity of the media resource. The verification process can be performed using a trusted identity provider within the network, such as a DID.
...
Based on the context of the implementation, the `typ` claim in the `header` of the JSON Web Token (JWT) MUST be included to establish the `format` of the multimedia resource associated, as per the [IANA MediaTypes](https://www.iana.org/assignments/media-types/media-types.xhtml) guidelines:
...
When using the [JSON Compact Serialization](https://www.rfc-editor.org/rfc/rfc7515#section-7.1) format, the "claims" field SHOULD use a [CID](https://docs.ipfs.tech/concepts/content-addressing/) value. This approach preserves the compact and lightweight design of the JWT signature while also giving it a deterministic nature. This enables it to be stored in raw blocks plus fast bitswap.
...
Additionally, this standard suggests the use of [DAG-JOSE](http://ipld.io.ipns/specs/codecs/dag-jose/spec/) as a complementary standard that MAY be used for serializing JSON Web Tokens (JWTs). [DAG-JOSE](http://ipld.io.ipns/specs/codecs/dag-jose/spec/) supports [General JSON Syntax](https://www.rfc-editor.org/rfc/rfc7515#section-7.2.1) serialization and provides a more secure and robust serialization format that is compatible with the [IPLD](https://ipld.io/) ecosystem.
