...
Both JWS and JWE can be serialized in different ways. The most common ways to serialize them are:
  ...
  2. **JSON Serialization**: The JWS JSON Serialization represents digitally signed or MACed  content as a JSON object. This representation is neither optimized for compactness nor URL-safe. There are two variations of this format:
      * General JSON Syntax, which uses multiple JSON objects to represent the payload and signature.
      * Flattened JSON Syntax, which is based upon the general syntax but flattens it, optimizing it for the single digital signature/MAC case.
