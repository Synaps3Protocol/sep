...
Both JWS and JWE can be serialized in different ways. The most common ways to serialize them are:
  ...
  2. **JSON Serialization**: This format separates the header, payload, and signature into separate JSON objects. There are two variations of this format: General JSON Syntax, which uses multiple JSON objects to represent the header, payload, and signature, and Flattened JSON Syntax, which combines the header and payload into the same JSON object.

When using the JSON compact serialization format, the "claims" field SHOULD use a CID value. This maintains the compact and lightweight design of the JWT signature and allows it to be stored in raw blocks. The following example demonstrates this:
...
Moreover, the standard proposes the use of [DAG-JOSE](http://ipld.io.ipns/specs/codecs/dag-jose/spec/) as a complementary standard that MAY be used for serializing JSON Web Tokens (JWTs), which provides a more secure and robust serialization format that supports JSON General Syntax serialization and binary encoding formats.
