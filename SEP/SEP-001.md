...
The purpose of this document is to propose a standard for transmitting and managing multimedia resources over decentralized systems. This standard aims to achieve interoperability, security and federation of such resources.

> JWTs represent a set of claims as a JSON object that is encoded in a JWS and/or JWE structure.

The proposed standard uses JSON Web Token (JWT) as the standard container for metadata. It extend JWT with additional claims to represent the structural, descriptive, and technical metadata of the resources.
...
> Interoperability is one of the main reasons behind the use of JWTs. JWT is a standard widely recognized and supported by many programming languages, frameworks, and tools. We believe that JSON Web Tokens (JWT) can encapsulate our current standard and evolve into a secure standard, through the implementation of JSON Web Signature ([RFC7515](https://www.rfc-editor.org/rfc/rfc7515)) and JSON Web Encrypted ([RFC7516](https://www.rfc-editor.org/rfc/rfc7516)).
...
JWT is used as a container structure for the metadata and MUST follow the structure defined in the [RFC7519](https://www.rfc-editor.org/rfc/rfc7519) specification.
...
2. **"d" descriptive metadata**
    ...
    * "name": It MUST be a string expressing the name of the multimedia resource.
...
JSON Web Tokens (JWTs) are a type of token that can be used for authentication and authorization purposes. There are two main types of JWTs: signed JWTs (also known as JWS, or JSON Web Signatures) and encrypted JWTs (also known as JWE, or JSON Web Encryption).

Both JWS and JWE can be serialized in different ways. The most common ways to serialize them are:

  1. JSON Compact Serialization: This is the default serialization format for JWTs. It combines the header, payload, and signature into a single string, separated by periods. This format is compact and easy to transmit over the wire.

  2. JSON Serialization: This format separates the header, payload, and signature into separate JSON objects. There are two variations of this format: General JSON Syntax, which uses multiple JSON objects to represent the header, payload, and signature, and Flattened JSON Syntax, which combines the header and signature into the same JSON object.

In the JSON compact serialization format, in order to maintain a compact and lightweight design of the JWT signature, it MUST use a CID as the value of the "claims" field. This can be seen in the example provided:
...
Additionally, this standard proposes [dag-jose](http://ipld.io.ipns/specs/codecs/dag-jose/spec/) as a complementary standard for serialization of the JSON Web Tokens (JWTs) used to transmit multimedia metadata over decentralized systems. DAG-JOSE provides a more robust and secure serialization format that supports JSON General Syntax serialization and binary encoding formats.
